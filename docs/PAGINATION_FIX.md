# Pagination repair and optional infinite scrolling — 2026.09.08.4

Release .3 restored Pinterest search after the HTTP 403 compatibility repair, and the operator confirmed that release working on the VPS. Its result parser could still discard the continuation token, leaving a populated gallery without a Next page link. Release .4 corrects that parser and adds optional automatic loading of subsequent pages.
...
The parser now reads the canonical token first. Explicit empty, terminal or malformed values end pagination; they cannot be overridden by a token from an older response shape. Legacy singular bookmark fields and named bookmark lists remain supported when the canonical field is absent. Tokens must be nonempty strings of at most 2048 bytes without whitespace or control characters. Known end markers and an immediate repeat of the requested token are not offered as a next page.

Parsed search-cache keys now use a `v2:` prefix before hashing the query/bookmark tuple. This prevents entries produced by the previous parser from hiding the corrected continuation link. Cache size, expiry and request limits remain in effect.
...
Local regression results and their exact scope are recorded in [AUDIT.md](../AUDIT.md) and [TEST_RESULTS.txt](TEST_RESULTS.txt). The development environment cannot run this release in Docker, contact Pinterest for live pagination, or verify scrolling in a real browser. No .4 VPS deployment was executed while preparing the package. Those checks must run in an environment with the required access; a local parser or script test does not substitute for them.

## Resumo em português

A .3 voltou a buscar imagens, mas podia perder o marcador da página seguinte ao procurar no local errado da resposta. A .4 lê `resource.options.bookmarks[0]` e usa novas chaves de cache para que resultados antigos sem continuação não escondam o link corrigido.
...
Antes da troca na VPS, o comando normal exige a primeira página de `architecture`, uma continuação válida e novas imagens na segunda página. A compilação Docker da .4, a paginação real e a verificação no navegador ainda dependem de execução em um ambiente com esse acesso.
